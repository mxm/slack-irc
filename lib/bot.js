...
function Bot(options) {
  ...
  //this.noticeOnIrcJoin = options.slackIrcJoinNotices || false;
  //this.noticeOnIrcLeave = options.slackIrcLeaveNotices || false;
...
Bot.prototype.attachListeners = function() {
  ...
  this.ircClient.on('invite', function(channel, from) {
    ...
    var joinMessage = ' joined the channel';
    var leaveMessage= ' left the channel';

    this.ircClient.on('join', function(channel, nick) {
        //if (this.noticeOnIrcJoin) {
           this.sendToSlackChannelEvent(this.nickname, channel, nick + joinMessage, 'good');
       // }
    }.bind(this));

    this.ircClient.on('part', function(channel, nick, arg) {
        //if (this.slackOptions.partNotices) {
           this.sendToSlackChannelEvent(this.nickname, channel, nick + leaveMessage, 'danger');
           //this.sendToSlack(this.nickname, channel, nick + leaveMessage);
        //}
    }.bind(this));
    ...
    this.ircClient.on('quit', function(nick, reason, channels, message) {
       // if (this.slackOptions.quitNotices) {
        ...
        for (index = 0; index < this.channels.length; index++) {
                this.sendToSlackChannelEvent(this.nickname, this.channels[index], nick + leaveMessage, 'danger');
                //this.sendToSlack(this.nickname, this.channels[index], nick + leaveMessage);
            }
       // }
    }.bind(this));
};
...
Bot.prototype.sendToSlack = function(author, channel, text) {
    this.sendToSlackWithAttachments(author, channel, text, []);
};

Bot.prototype.sendToSlackChannelEvent = function(author, channel, text, color) {
    var attachments = [
        {
            "text" : text,
            "color": color
    ...
    this.sendToSlackWithAttachments(author, channel, ' ', attachments);
};

Bot.prototype.sendToSlackWithAttachments = function(author, channel, text, attachments) {
