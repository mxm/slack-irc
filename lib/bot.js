...
Bot.prototype.attachListeners = function() {
  ...
  this.ircClient.on('invite', function(channel, from) {
    ...
    var joinMessage = '_Joined the channel._';
    var leaveMessage= '_Left the channel._';

    this.ircClient.on('join', function(channel, nick) {
        //if (this.noticeOnIrcJoin) {
            this.sendToSlack(this.nickname, channel, joinMessage);
       // }
    }.bind(this));

    this.ircClient.on('part', function(channel, nick, arg) {
        //if (this.slackOptions.partNotices) {
            var msg = '__' + nick + ' has left the channel _';
            this.sendToSlack(this.nickname, channel, leaveMessage);
        //}
    }.bind(this));
    ...
    this.ircClient.on('quit', function(nick, reason, channels, message) {
       // if (this.slackOptions.quitNotices) {
            ...
            for (index = 0; index < this.channels.length; index++) {
                this.sendToSlack(this.nickname, this.channels[index], leaveMessage);
            }
       // }
    }.bind(this));
};
